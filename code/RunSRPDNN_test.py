...
if __name__ == "__main__":
	...
	if args.gen_on_the_fly:
		dataset_train = at_dataset.RandomMicSigDataset(
			...
			T60 = Parameter(0.1, 0.2),					# Random reverberation times from 0.2 to 1.3 seconds
		...
		dataset_test = at_dataset.RandomMicSigDataset( 
			...
			source_state = args.source_state,
			room_sz = Parameter([3,3,2.5], [10,8,6]),
			T60 = Parameter(0.1, 0.2), # T60 = Parameter(0.2, 1.3),
			abs_weights = Parameter([0.5]*6, [1.0]*6),
			array_setup = array_setup,
			array_pos = Parameter([0.1,0.1,0.1], [0.9,0.9,0.5]),
	...
	if (args.train):
		...
		if dataset_mode == 'simulate':
			...
			# pred_np, gt_np = pred[0]['doa'].cpu().numpy(), gt[0]['doa'].cpu().numpy()
			# np.save('pred.npy', pred_np)
			# np.save('gt.npy', gt_np)

			# loss_test, metric_test = learner.test_epoch_tqdm(dataloader_test, return_metric=True)
			# print('Test loss: {:.4f}, Test MDR: {:.2f}%, Test FAR: {:.2f}%, Test MAE: {:.2f}deg'.\
			# 			format(loss_test, metric_test['MDR']*100, metric_test['FAR']*100, metric_test['MAE']) )
			# print("\n")

			# gt_angles, pred_angles = [],[]
			# for index in range(len(dataloader_test)):
			# 	gt_angles.append(gt[index]['doa'][0,0,0,0])
			# 	pred_angles.append(pred[index]['doa'][0,0,0,0])

			gt_angles = torch.tensor([gt[index]['doa'][0,0,0,0] for index in range(len(dataloader_test))])
			pred_angles = torch.tensor([pred[index]['doa'][0,0,0,0] for index in range(len(dataloader_test))])
			...
			bins = np.linspace(0, 180, 181)  # 180个框，每个框1度
			...
			# plt.plot(bin_centers, average_mse_per_bin,'.',label='Average MSE per Degree')
			plt.plot(bin_centers, average_rmse_per_bin,'.',label='Average RMSE per Degree')
			plt.xlabel('Angle (Degreee)')
			plt.ylabel('Localization RMSE')
			plt.title('RMSE by Angle')
			plt.legend()
			plt.grid(True)
			save_fig_name = './results/' + args.array_name  + '_rmse_by_angle_1.png' 
			plt.savefig(save_fig_name, dpi=300)  # 保存为PNG文件，分辨率为300DPI
			plt.show()
